import { CloseRoundIcon, EditIcon, SearchIcon, TrashIcon } from "@/components/icon";
import { Button } from "@/components/ui/button";
import type { Category } from "@/types/category";

type CategoryManagementContentProps = {
  categories: Category[];
  loading: boolean;
  error: string | null;
  searchInput: string;
  ...
  onOpenDeleteModal: (categoryId: number) => void;
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[760px] text-left">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={2} className="px-4 py-8 text-center text-body-2 text-brown-400">
                  กำลังโหลด...
                </td>
              </tr>
            ) : error ? (
              <tr>
                <td colSpan={2} className="px-4 py-8 text-center text-body-2 text-red-600">
                  {error}
                </td>
              </tr>
            ) : categories.length === 0 ? (
              ...
              categories.map((category) => (
                <tr key={category.id} className="border-b border-brown-200 text-body-2 text-brown-500">
                  ...
                  <td className="px-4 py-4">
                    <div className="flex justify-end gap-3">
                      ...
                      >
                        <img src={EditIcon} alt="Edit category" className="h-4 w-4" />
                      </button>
                      <button
                        type="button"
                        className="cursor-pointer opacity-80 transition hover:opacity-100"
                      ...
                      >
                        <img src={TrashIcon} alt="Delete category" className="h-4 w-4" />
                      </button>
                    </div>
                  </td>
                </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-[640px] rounded-[32px] bg-white px-10 py-8 shadow-xl">
            ...
            <div className="text-center">
              <h3 className="text-headline-3 text-brown-600">Delete category</h3>
